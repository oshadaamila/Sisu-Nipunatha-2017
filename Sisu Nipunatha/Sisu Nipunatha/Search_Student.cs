...
namespace Sisu_Nipunatha
{
    public partial class Search_Student : Form
    {
        ...
        private void numericUpDown1_ValueChanged(object sender, EventArgs e)
        {
            ...
            {
                MySqlDataAdapter sda = new MySqlDataAdapter("select studentstable.StudentID,studentstable.Name,studentstable.Birthday,studentstable.dahampasala,studentstable.Name_english,competitiontable.competitionname,competitiontable.grade,studentstable.overage  FROM studentstable INNER JOIN competitiontable ON studentstable.CompetitionID=competitiontable.CompetitionID where studentstable.studentID like'" + numericUpDown1.Value.ToString() + "%" + "';", SqlCon.con);
            ...
            {
                MySqlDataAdapter sda = new MySqlDataAdapter("select studentstable.StudentID,studentstable.Name,studentstable.Birthday,studentstable.dahampasala,studentstable.Name_english,competitiontable.competitionname,competitiontable.grade,studentstable.overage  FROM studentstable INNER JOIN competitiontable ON studentstable.CompetitionID=competitiontable.CompetitionID;", SqlCon.con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                dataGridView1.DataSource = dt;
                dataGridView1.Update();
	...
	catch (Exception )
	{
		...
		MessageBox.Show("ඉවත් කිරීමට නොහැක වෙනත් දත්ත හා සම්බන්ධතා පවතී","Relationship Error",MessageBoxButtons.OK,MessageBoxIcon.Warning);
         ...
         public void updateDatagridview()
         {
             ...
             MySqlDataAdapter sda = new MySqlDataAdapter("select studentstable.StudentID,studentstable.Name,studentstable.Birthday,studentstable.dahampasala,studentstable.Name_english,competitiontable.competitionname,competitiontable.grade,studentstable.overage  FROM studentstable INNER JOIN competitiontable ON studentstable.CompetitionID=competitiontable.CompetitionID order by studentid;", SqlCon.con);
         ...
         private void checkBox1_CheckedChanged(object sender, EventArgs e)
         {
             if (checkBox1.Checked)
             {
                 ...
                 MySqlDataAdapter sda = new MySqlDataAdapter("select studentstable.StudentID,studentstable.Name,studentstable.Birthday,studentstable.dahampasala,studentstable.Name_english,competitiontable.competitionname,competitiontable.grade,studentstable.overage  FROM studentstable INNER JOIN competitiontable ON studentstable.CompetitionID=competitiontable.CompetitionID where overage= 1 order by studentid;", SqlCon.con);
